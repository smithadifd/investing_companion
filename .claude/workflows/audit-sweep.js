...
const HINTS = {
  ...
  'correctness-bugs': `BACKEND: analysis/scoring logic in services/technical.py, entry_zones.py, trade_readiness.py, exposure.py, ratio.py (null/divide-by-zero, boundary conditions, currency/percent handling); services/alert.py + tasks/alerts.py alert evaluation (residual: ratio-percent alerts with forex legs still no-op — resolves by exact Equity.symbol, bypassing yahoo.py normalize_symbol; alert.py ~980-1009); services/data_providers/ aggregator fallback ordering + dedup; Celery beat schedules in tasks/celery_app.py. AI advisor contract: services/context_pack.py (UNSUPPORTED_FEATURES authority) + schemas/context_pack.py (SCHEMA_VERSION / ADVISOR_ACTIONS_VERSION) + services/context_pack_outbox.py + services/handoff.py — version-stamp correctness. FRONTEND: frontend/src/lib/hooks/** state transitions, optimistic-update/rollback in mutation hooks.`,
  'docs-accuracy': `Verify AGENTS.md (repo map vs actual dirs, env-var table vs backend/app/core/config.py, demo-mode contract vs core/dependencies.py require_not_demo, provider table vs services/data_providers/). Advisor contract docs are load-bearing: docs/api/handoff-schema.md (SCHEMA_VERSION / ADVISOR_ACTIONS_VERSION) and docs/api/advisor-actions.md (ADVISOR_ACTIONS_VERSION) must match backend/app/schemas/context_pack.py; docs/api/API_SPECIFICATION.md vs endpoints/**. Flag undocumented endpoints/services and stale docs-site/ pages.`,
}
