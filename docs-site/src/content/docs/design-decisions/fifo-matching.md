...
`_recalculate_pairs` and its FIFO matching are covered across several files in `backend/tests/test_services/`, not one single `test_trade_service.py`: `test_trade_fifo_tiebreak.py` (same-timestamp ordering), `test_trade_fees.py` (fee netting into `realized_pnl`), `test_trade_positions.py` (per-account partitioning, e.g. `test_fifo_matching_stays_within_account`), `test_open_lots.py` (the read-only open-lot walk), and `test_trade_journal_pair_order.py` (pair ordering).
